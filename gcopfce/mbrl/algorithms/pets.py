...
def train(
    ...
    training_reward_fn = cfg.overrides.get("training_reward_fn", None)
    if training_reward_fn and training_reward_fn in additional_modules.keys():
        print(f"Overriding reward signal: {training_reward_fn}")
        on_step_callback = lambda x: additional_modules[training_reward_fn].on_step(x)
        reset_callback = lambda x: additional_modules[training_reward_fn].on_reset(x)
    else:
        on_step_callback, reset_callback = None, None

    while sv.env_steps < cfg.overrides.num_steps:
        obs = env.reset()
        if isinstance(obs, tuple):  # Gymnasium API fix
            obs = obs[0]
        agent.reset()
        video_recorder.init()
