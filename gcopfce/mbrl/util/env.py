...
import gym.wrappers
import omegaconf
from mbrl.env.parking_env import ParkingEnv
...
def _get_term_and_reward_fn(
    ...
    """Returns a specified reward and termination function, handling None safely."""
    import mbrl.env

    term_fn = None
    reward_fn = None

    # Only load if provided
    if getattr(cfg.overrides, "term_fn", None):
        term_fn = getattr(mbrl.env.termination_fns, cfg.overrides.term_fn)

    if getattr(cfg.overrides, "reward_fn", None):
        reward_fn = getattr(mbrl.env.reward_fns, cfg.overrides.reward_fn)
    elif term_fn is not None:
...
def make_env(
    ...
    if "maze___" in cfg.overrides.env:
        ...
        term_fn, reward_fn = _get_term_and_reward_fn(cfg)
    elif "parking" in cfg.overrides.env:
        env = ParkingEnv()
        term_fn, reward_fn = _get_term_and_reward_fn(cfg)
        if term_fn is None:
