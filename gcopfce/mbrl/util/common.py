...
def step_env_and_add_to_buffer(
    ...
    if grow_buffer:
            ...
            if isinstance(o, dict):
                ...
                for k, v in o.items():
                    if isinstance(v, (list, tuple)):
                        for sub_v in v:
                            parts.append(np.ravel(sub_v))
                    else:
                        parts.append(np.ravel(v))
                return np.concatenate(parts)

            # Handle tuples/lists of arrays
        ...
        #print("\n[DEBUG] obs type:", type(obs))
        #print("[DEBUG] obs content keys:", obs.keys() if isinstance(obs, dict) else None)
        # if isinstance(obs, dict):
        #     for k, v in obs.items():
        #         print(f"[DEBUG]   {k}: type={type(v)}, shape={np.shape(v)}")
        # print("[DEBUG] flatten_obs result shape:", np.shape(flatten_obs(obs)))

        #replay_buffer.add(flatten_obs(obs), action, flatten_obs(next_obs), reward, done)
        # --- Fix: handle (obs, info) tuples from Gymnasium reset() ---
        def extract_obs(o):
            # If reset() or step() returns (obs, info), keep only obs
            if isinstance(o, tuple) and len(o) == 2 and isinstance(o[1], dict):
                return o[0]
            return o

        obs_clean = extract_obs(obs)
        next_obs_clean = extract_obs(next_obs)
        replay_buffer.add(flatten_obs(obs_clean), action, flatten_obs(next_obs_clean), reward, done)
